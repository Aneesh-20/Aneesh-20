<!-- PREMIUM EXECUTIVE GITHUB README // PALETTE: SUNSET GLOW -->
<!-- Brand Colors: Deep Cocoa (#1A0F0D), Sunset Orange (#FF5E36), Ember Gold (#FFAE19), Crisp Snow (#F8F1E5) -->

<!-- 1. HEADER BANNER -->
<p align="center">
  <img src="https://raw.githubusercontent.com/Aneesh-20/Aneesh-20/refs/heads/main/banner.svg" alt="Aneesh Tech Banner" width="100%" style="border-radius: 12px;" />
</p>

<!-- 2. ANIMATED TERMINAL HEADER -->
<div align="center">
  <a href="https://github.com/Aneesh-20" target="_blank">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=26&pause=1000&color=FF5E36&center=true&vCenter=true&width=800&height=50&lines=ANEESH+T+NARENDARAN+%7C+DATA+ANALYST;TURNING+COMPLEX+DATA+INTO+ACTIONABLE+INSIGHTS;ADVANCED+PREDICTIVE+MODELING+%26+BI" alt="Aneesh Typing Header" />
  </a>
</div>

<!-- 3. MINIMALIST NETWORKING & SOCIALS -->
<p align="center">
  <a href="https://linkedin.com/in/www.linkedin.com/in/aneeshn20" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-1A0F0D?style=flat-square&logo=linkedin&logoColor=FF5E36" alt="LinkedIn" /></a>&nbsp;
  <a href="mailto:aneesh.narendaran@gmail.com"><img src="https://img.shields.io/badge/-Email-1A0F0D?style=flat-square&logo=gmail&logoColor=FF5E36" alt="Email" /></a>&nbsp;
  <a href="https://YOUR_PORTFOLIO.com" target="_blank"><img src="https://img.shields.io/badge/-Portfolio-1A0F0D?style=flat-square&logo=githubpages&logoColor=FFAE19" alt="Portfolio" /></a>&nbsp;
  <a href="https://twitter.com/YOUR_TWITTER" target="_blank"><img src="https://img.shields.io/badge/-Twitter-1A0F0D?style=flat-square&logo=x&logoColor=F8F1E5" alt="Twitter" /></a>
</p>


---

<!-- 4. DASHBOARD SPLIT GRID: ABOUT + LIVE STATS -->
<table align="center" width="100%" style="border-collapse: collapse; border: none;">
  <tr>
    <!-- Left Panel: Core Profile -->
    <td width="55%" valign="top" style="border: none; padding-right: 20px;">
      <br/>
      <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=16&color=FFAE19&width=450&lines=%22In+God+we+trust.+All+others+must+bring+data.%22" alt="Deming Quote" />
      <br/><br/>
      <p>👋 <b>Welcome.</b> I specialize in architectural data analytics, designing end-to-end automated pipelines, and unearthing micro-trends within massive datasets.</p>
      <ul>
        <li>🚀 <b>Active Initiatives:</b> Engineered timetable clustering algorithms and student performance analytics.</li>
        <li>⚡ <b>Core Focus:</b> Machine Learning optimizations, statistical forecasting models, and interactive dashboard systems.</li>
        <li>🛠️ <b>Architecture:</b> Scaling pipelines via Python, SQL query refactoring, and Apache Spark data warehouses.</li>
      </ul>
    </td>
    <!-- Right Panel: High-Contrast Metrics Engine -->
    <td width="45%" align="center" valign="middle" style="border: none;">
      <img src="https://github-readme-stats.vercel.app/api?username=Aneesh-20&show_icons=true&theme=calm&bg_color=1A0F0D&title_color=FF5E36&icon_color=FFAE19&text_color=F8F1E5&border_color=FF5E36&hide_border=false&border_radius=10" alt="Aneesh Dev Stats" width="100%"/>
    </td>
  </tr>
</table>

---

<!-- 5. LUXURY TECH MATRIX (CLEAN GRAPHICS) -->
### 🛠️ Core Technology Matrix

<table align="center" width="100%" style="border-collapse: collapse; text-align: center;">
  <tr>
    <td style="padding: 10px;"><b>Analytics & BI</b></td>
    <td style="padding: 10px;"><b>AI & Core ML</b></td>
    <td style="padding: 10px;"><b>Architecture</b></td>
    <td style="padding: 10px;"><b>Databases</b></td>
  </tr>
  <tr>
    <td style="padding: 12px; background-color: #1A0F0D; border-radius: 8px;">
      <img src="https://skillicons.dev/icons?i=python,pandas,numpy,r,tableau,powerbi&perline=3" alt="Analytics Stack" />
    </td>
    <td style="padding: 12px; background-color: #1A0F0D; border-radius: 8px;">
      <img src="https://skillicons.dev/icons?i=tensorflow,pytorch,sklearn,opencv&perline=2" alt="ML Stack" />
    </td>
    <td style="padding: 12px; background-color: #1A0F0D; border-radius: 8px;">
      <img src="https://skillicons.dev/icons?i=docker,fastapi,git,aws,azure,linux&perline=3" alt="Infrastructure Stack" />
    </td>
    <td style="padding: 12px; background-color: #1A0F0D; border-radius: 8px;">
      <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,sqlite&perline=2" alt="Database Stack" />
    </td>
  </tr>
</table>

---

<!-- 6. FEATURED REPOSITORIES (CUSTOM COMPACT BORDERS) -->
### 📂 Production Showcases

<p align="center">
  <a href="https://github.com/Aneesh-20/Timetable-Cluster">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Aneesh-20&repo=Timetable-Cluster&bg_color=1A0F0D&title_color=FF5E36&icon_color=FFAE19&text_color=F8F1E5&border_color=1A0F0D" alt="Timetable Cluster Repository" height="115" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/Aneesh-20/Gap_Detection-Student_Analysis">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Aneesh-20&repo=Gap_Detection-Student_Analysis&bg_color=1A0F0D&title_color=FF5E36&icon_color=FFAE19&text_color=F8F1E5&border_color=1A0F0D" alt="Gap Detection Repository" height="115" />
  </a>
</p>

---

<!-- 7. ADVANCED LIVE INSIGHTS & ANALYTICS TIMELINE -->
### 📈 Real-Time Engine Metrics

<p align="center">
  <a href="https://github.com/Aneesh-20">
    <img src="https://github-readme-activity-graph.vercel.app/graph?username=Aneesh-20&bg_color=1A0F0D&color=FFAE19&line=FF5E36&point=F8F1E5&area=true&area_color=FF5E36&hide_border=false&border_color=FF5E36&radius=10&custom_title=DATALINE%20//%20CONTRIBUTION%20TIMELINE" alt="Aneesh Contribution Analytics" width="100%" />
  </a>
</p>

<table align="center" width="100%">
  <tr>
    <td width="50%" align="center" style="border: none;">
      <img src="https://github-readme-streak-stats.herokuapp.com/?user=Aneesh-20&theme=calm&background=1A0F0D&ring=FF5E36&fire=FFAE19&stroke=1A0F0D&currStreakLabel=FFAE19&sideLabels=F8F1E5" alt="Commit Streak Metrics" width="100%"/>
    </td>
    <td width="50%" align="center" style="border: none;">
      <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Aneesh-20&layout=compact&theme=calm&bg_color=1A0F0D&title_color=FF5E36&icon_color=FFAE19&text_color=F8F1E5&border_color=1A0F0D" alt="Top Languages Map" width="100%" />
    </td>
  </tr>
</table>

<!-- Smooth, premium custom vector footer -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=FF5E36&height=15&section=footer&radius=4" width="100%" />
</p>
